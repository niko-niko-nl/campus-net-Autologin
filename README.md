...
开机后自动登录校园网，掉线自动重连，检测到游戏在运行时会暂停。

走的是学校门户那套网页认证（`http://10.130.128.9/eportal`，深澜/锐捷的 eportal），不模拟点击浏览器，直接调门户自己的接口。
...
下载 ZIP，解压，双击 `install.bat`，输入账号和密码。

**要先解压，不要在压缩包的预览窗口里直接双击。** 那种情况下 Windows 只是把文件临时解到别处，脚本找不到跟自己放在一起的其他文件，安装会失败。
...
以后开机会自动登录。想立刻验证一下效果，双击 `run.bat`，它会打印当前联网状态并强制登录一次。
...
这只能降低暴露面，不能保证反作弊一定不误判。代码里没有任何作弊特征（不注入进程、不挂钩子、不装驱动、也不模拟键鼠），但各家反作弊内部怎么判的没法核实。如果你的游戏反作弊特别严，最稳妥的做法是玩之前把任务停掉：
...
这类工具被误报比较常见，没法保证一定不触发。它做的事情（定时运行脚本、发起网络请求、保存凭据）跟某些恶意软件有重叠。

所以这里不发编译好的 exe，只发可读的 PowerShell 源码：没有混淆、没有用 base64 藏载荷、不注入进程、不挂钩子、不装驱动、不写注册表。
...
火绒在右上角菜单 → 信任区 → 添加目录；360 在设置 → 白名单；Windows Defender 在 Windows 安全中心 → 病毒和威胁防护 → 排除项。建议加白名单，不要直接关闭杀软。

在火绒 6.0.11.3（防护全开）上的实测：带“来自互联网”标记的下载包安装正常；装完目录静置 30 秒，没有文件被删或被改（逐个比对 SHA256）；计划任务连续触发 5 次全部成功；隔离区零新增。

对照组是同一台机器编译出的自解压 `CampusNet-Setup.exe`，5 秒内被静默隔离，没有任何提示。隔离区历史里 13 条记录全是 exe，没有一条是脚本。
